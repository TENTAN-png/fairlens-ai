 ...
 * All model inspection operations require the ML backend to be running.
 */
...
// ── Model Inspection (Real ML Backend Only — No Simulation Fallbacks) ─────────
export async function auditModel(datasetFile, modelFile, config) {
  ...
  try {
    ...
    throw new Error(`Model audit failed: ${error.message}. Ensure the ML backend is running on ${BACKEND_URL}`);
  }
}
...
export async function checkModelConsistency(datasetFile, modelFile, config) {
  ...
  try {
    ...
    throw new Error(`Consistency check failed: ${error.message}. Ensure the ML backend is running on ${BACKEND_URL}`);
  }
}
...
export async function explainModel(datasetFile, modelFile, config) {
  ...
  try {
    ...
    throw new Error(`Explainability analysis failed: ${error.message}. Ensure the ML backend is running on ${BACKEND_URL}`);
  }
}
...
export async function compareModels(datasetFile, modelAFile, modelBFile, config) {
  ...
  try {
    ...
    throw new Error(`Model comparison failed: ${error.message}. Ensure the ML backend is running on ${BACKEND_URL}`);
  }
}
...
export async function mitigateModel(datasetFile, modelFile, config) {
  ...
  try {
    ...
    throw new Error(`Bias mitigation failed: ${error.message}. Ensure the ML backend is running on ${BACKEND_URL}`);
  }
}
